  ...
  <table class="table table-bordered text-dark" id="dataTable" width="100%" cellspacing="0">
    <thead>
      <tr>
        ...
        <th class="text-center" colspan="7">Menerangkan dengan sebenarnya bahwa</th>
    ...
    <tbody>
      <?php $no = 1;
      foreach ($views_suket_non_kk as $data) { ?>
        <tr>
          <td><?= $no ?></td>
          <td><?= $data['no_surat'] ?></td>
          <td><?= $data['nama_p1'] ?></td>
          ...
          <td class="text-center">
            <button type="button" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#ubah<?= $data['id_suket_non_kk'] ?>">
              <i class="bi bi-pencil-square"></i> Ubah
            </button>
            <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#hapus<?= $data['id_suket_non_kk'] ?>">
              <i class="bi bi-trash3"></i> Hapus
            </button>
            <a href="export-non-kk?id=<?= $data['id_suket_non_kk'] ?>" target="_blank" class="btn btn-primary btn-sm" rel="noopener noreferrer"><i class="bi bi-printer"></i> Cetak</a>

            <div class="modal fade" id="ubah<?= $data['id_suket_non_kk'] ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
              <div class="modal-dialog">
                <div class="modal-content">
                  ...
                  <form action="" method="post">
                    <input type="hidden" name="id_suket_non_kk" value="<?= $data['id_suket_non_kk'] ?>">
                    ...
                    <div class="modal-body">
                      ...
                      <div class="form-group">
                        ...
                        <select name="pekerjaan_p2" class="form-control" id="pekerjaan_p2<?= $data['id_suket_non_kk'] ?>" onchange="showInput<?= $data['id_suket_non_kk'] ?>()">
                      ...
                      <div class="form-group" id="inputManual<?= $data['id_suket_non_kk'] ?>" style="display: none;">
                      ...
                      <script>
                        function showInput<?= $data['id_suket_non_kk'] ?>() {
                          var select = document.getElementById("pekerjaan_p2<?= $data['id_suket_non_kk'] ?>");
                          var inputManual = document.getElementById("inputManual<?= $data['id_suket_non_kk'] ?>");
                      ...
                      <div class="form-group">
                        ...
                        <input type="text" name="kewarganegaraan" value="<?= $data['kewarganegaraan'] ?>" class="form-control" id="kewarganegaraan" minlength="3" required>
                      </div>
                      <div class="form-group">
                    ...
                    <div class="modal-footer justify-content-center border-top-0">
                      ...
                      <button type="submit" name="edit_suket_non_kk" class="btn btn-warning btn-sm">Ubah</button>
                    </div>
                  </form>
                </div>
              </div>
            ...
            <div class="modal fade" id="hapus<?= $data['id_suket_non_kk'] ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
              <div class="modal-dialog">
                <div class="modal-content">
                  <div class="modal-header border-bottom-0 shadow">
                  ...
                  <form action="" method="post">
                    ...
                    <div class="modal-footer justify-content-center border-top-0">
                      <button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal">Batal</button>
                      <button type="submit" name="delete_suket_non_kk" class="btn btn-danger btn-sm">hapus</button>
                    </div>
                  </form>
                </div>
              </div>
